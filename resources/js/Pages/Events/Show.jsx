...
export default function Show({ event }) {
    ...
    return (
        <DashboardLayout>
            <Head title={`Event Details: ${event?.title || 'Event'}`} />
            ...
            <div className="max-w-[1200px]">
                ...
                <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        ...
                        <h1 className="text-5xl font-black text-gray-900 tracking-tight capitalize">{safe(event?.title)}</h1>
                    </div>
                    <div className="flex items-center gap-3">
                        <Link
                            href={event?.id ? route('events.edit', event.id) : '#'}
                    ...
                    <div className="space-y-6">
                            ...
                            <div className="space-y-8">
                                <div>
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm capitalize">{event?.start_time ? formatDate(event.start_time).toLowerCase() : 'date tbd'}</p>
                                            <p className="text-slate-400 text-xs mt-1">{event?.start_time ? formatTime(event.start_time).toLowerCase() : 'time tbd'} — {event?.end_time ? formatTime(event.end_time).toLowerCase() : 'time tbd'}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm capitalize">{safe(event?.venue?.name) || 'unassigned location'}</p>
                                            <p className="text-slate-400 text-xs mt-1 truncate max-w-[180px]">{safe(event?.venue?.location) || 'no address available'}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-3">
                                         <div className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center text-xl">🏀</div>
                                          <div>
                                            <p className="font-bold text-sm capitalize">{safe(event?.team?.name) || 'open registration'}</p>
                                            {event?.opponent_team && (
                                                <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">vs {safe(event.opponent_team?.name)}</p>
                                            )}
                                         </div>
                                    </div>
                                </div>
                        ...
                        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
                             ...
                             <div className="flex items-center gap-3">
                                 ...
                                 <div>
                                     <p className="text-sm font-bold text-gray-900 capitalize">{safe(event?.user?.name)}</p>
